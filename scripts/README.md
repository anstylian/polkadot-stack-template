...
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `start-dev.sh`                  | Builds the runtime, generates `blockchain/chain_spec.json`, and starts a single local omni-node on the resolved Substrate RPC port using dev sealing.                                                                                                                           | Use this when you only need the fastest local pallet/runtime loop. On stable2603, this mode does not expose Statement Store RPCs.                                |
...
| `download-sdk-binaries.sh`      | Downloads `polkadot` plus `polkadot-prepare-worker` / `polkadot-execute-worker` (required beside relay `polkadot`), `polkadot-omni-node`, and `eth-rpc` from the stable2603 release into `./bin/` (gitignored).                                                                 | Use this to prefetch SDK binaries, or rely on the same download step from `common.sh` unless disabled.                                                           |
